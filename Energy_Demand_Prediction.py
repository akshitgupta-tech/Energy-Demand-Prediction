# -*- coding: utf-8 -*-
"""Another copy of VMD_new_final_02.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1WGDkbrAZgjAkymGbxf5NeYke2d3ajGai
"""
...
plt.tight_layout()
plt.savefig('/content/01_timeseries_analysis.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('/content/02_distribution_analysis.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('/content/03_hourly_daily_patterns.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('/content/04_acf_trend_analysis.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('/content/05_outliers_analysis.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('/content/06_regional_comparison.png', dpi=300, bbox_inches='tight')
plt.show()
...
# ------------------- PLOT VMD IMFs (PASTE HERE) -------------------
import os
import matplotlib.dates as mdates

out_dir = '/content/gdrive/My Drive/VMD_plots'
os.makedirs(out_dir, exist_ok=True)

# time index (datetime index already present)
time_index = df_train_full_vmd.index

# collect mode columns
modes = [c for c in df_train_full_vmd.columns if c.startswith('IMF_')]
n_modes = len(modes)

# 1) Stacked plot: original + all IMFs
fig, axes = plt.subplots(n_modes + 1, 1, figsize=(16, 2*(n_modes+1)), sharex=True)
axes[0].plot(time_index, df_train_full_vmd['Load'].values, color='black', linewidth=0.6)
axes[0].set_ylabel('Load (MW)')
axes[0].set_title('Original Signal (National Load)')
axes[0].grid(alpha=0.25)

for i, m in enumerate(modes):
    axes[i+1].plot(time_index, df_train_full_vmd[m].values, linewidth=0.7)
    axes[i+1].set_ylabel(m)
...
plt.tight_layout()
stacked_fname = os.path.join(out_dir, 'VMD_all_modes_stacked.png')
plt.savefig(stacked_fname, dpi=300, bbox_inches='tight')
plt.show()
print(f"Saved: {stacked_fname}")

# 2) Individual large plots for selected IMFs (IMF3, IMF5 by default if available)
selected_idxs = []
if n_modes >= 3: selected_idxs.append(2)   # IMF_3 (zero-based index 2)
if n_modes >= 5: selected_idxs.append(4)   # IMF_5

for idx in selected_idxs:
    col = modes[idx]
    plt.figure(figsize=(18,3.5))
    plt.plot(time_index, df_train_full_vmd[col].values, linewidth=0.8)
    plt.title(f'{col} (VMD mode)')
    plt.xlabel('Datetime')
    plt.ylabel('Amplitude')
    plt.grid(alpha=0.25)
    fname = os.path.join(out_dir, f'{col}_large.png')
    plt.tight_layout()
    plt.savefig(fname, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"Saved: {fname}")

# 3) Energy share bar plot (variance)
imf_array = np.vstack([df_train_full_vmd[m].values for m in modes])
imf_energy = np.var(imf_array, axis=1)
residual = df_train_full_vmd['Load'].values - imf_array.sum(axis=0)
res_energy = np.var(residual)
labels = modes + ['Residual']
energy_vals = np.concatenate([imf_energy, [res_energy]])

plt.figure(figsize=(10,5))
plt.bar(labels, energy_vals)
plt.xticks(rotation=45)
plt.ylabel('Variance (energy)')
plt.title('Energy share: IMFs vs Residual')
energy_fname = os.path.join(out_dir, 'VMD_IMF_energy_share.png')
plt.tight_layout()
plt.savefig(energy_fname, dpi=300, bbox_inches='tight')
plt.show()
print(f"Saved: {energy_fname}")
# ------------------------------------------------------------------
